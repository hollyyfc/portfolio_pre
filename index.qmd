...
::: {style="font-size: 70%;"}
> _Does prior math performance impact students' current math scores (controlling for class type), and how does this effect vary across schools?_

:::

$$
\scriptsize \text{MathScore}_{ij} = \beta_0 + \beta_1(\text{ClassType}_{ij}) + \beta_2(\text{MathOld}_{ij}) + a_{0j} + a_{1j}(\text{MathOld}_{ij}) + \epsilon_{ij}
$$

- $\tiny i \text{ indexes students},\ j \text{ indexes schools.}$
- $\tiny \beta_0, \beta_1, \beta_2 \text{ are fixed intercept and slopes.}$
- $\tiny \mathbf{a}_j = \left(a_{0j}, a_{1j} \right) \sim N(\mathbf{0,\Psi}) \text{ is a vector of random intercept for school and random slope for MathOld.}$
- $\tiny \epsilon_{ij} \sim N(0, \sigma^2) \text{ is the residual (within-school heterogeneity).}$
...
## <span style="font-size: 55px;">`assumption_check()`</span>
...
<div style="display: flex; flex-direction: column; justify-content: flex-end; height: 65vh;">
