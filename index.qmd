...
$$
\scriptsize \text{MathScore}_{ij} = \beta_0 + \beta_1(\text{ClassType}_{ij}) + \beta_2(\text{MathOld}_{ij}) + b_{0j} + b_{1j}(\text{MathOld}_{ij}) + \epsilon_{ij}
$$

- $\tiny i \text{ indexes students},\ j \text{ indexes schools.}$
- $\tiny \beta_0, \beta_1, \beta_2 \text{ are fixed intercept and slopes.}$
- $\tiny b_{0j} \sim N(0, \tau^2_{0}) \text{ is a random intercept for each school.}$
- $\tiny b_{1j} \sim N(0, \tau^2_{1}) \text{ is a random slope for MathOld in each school.}$
